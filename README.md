...
`PANOPTICON_ORACLES_BASE` and `PANOPTICON_PAYMENTS_BASE` are **env stubs only** (server-side). Empty (default) means no hop — keep today’s browser-first SIWE / ENS / ERC-1271, no hard paywall, no hard fail on `/feed`. Reuse `PANOPTICON_TENANT_ID` + `PANOPTICON_API_KEY` when consume is unlocked. Never `NEXT_PUBLIC_*`. Never Gun. Later hops stay held: Panopticon `POST /api/v1/oracles/v0/attest` and `POST /api/v1/payments/v0/receipt` (intent optional). Contracts: FyberLabs/panopticon [`oracles-attest-v0.md`](https://github.com/FyberLabs/panopticon/blob/main/products/oracles/docs/oracles-attest-v0.md), [`payments-access-v0.md`](https://github.com/FyberLabs/panopticon/blob/main/products/payments/docs/payments-access-v0.md). Ops: [docs/oracles-and-payments-prep.md](docs/oracles-and-payments-prep.md).
