...
The public [lab feed](/feed) is a GunDB graph. This slice’s seeder and `GET /api/feed` snapshot are a bootstrap cache (so the graph is not empty and App Service is not the chat server); the end-state is a mostly browser-to-browser Gun mesh. A user is an identity graph of public wallet traces; extra proofs (email, phone, digital KYC) are held claims the user grants visibility to — a lighter SociACL Check, not government KYC. See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).
