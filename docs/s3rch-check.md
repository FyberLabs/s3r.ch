...
| Name | Meaning |
| --- | --- |
| `object` | `GunFeedNode` at `s3rch/items/<encodeKey(id)>`, `GunRoomNode` at `s3rch/rooms/<encodeKey(id)>`, `GunChatNode` at `s3rch/rooms/<encodeKey(room)>/chat/<encodeKey(id)>`, `GunPresenceNode` at `s3rch/rooms/<encodeKey(room)>/presence/<encodeKey(address)>`, or a Gun-native claim linked from `s3rch/users/{wallet}` |
