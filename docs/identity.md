...
- **Panopticon oracles attest hop** (`POST /api/oracles/attest`). SIWE cookie, then the Next server hops `POST /api/v1/oracles/v0/attest` with `PANOPTICON_ORACLES_BASE` + shared tenant/key. Empty env is fail-soft (no invented credentials, no grant from a missing digest). `/verify` later. Payments stay held. Not visitor UI.
...
Production sets `IDENTITY_SESSION_SECRET` on the Azure App Service from Key Vault (`terraform/s3rch` in FyberLabs/infra). Build and CI must not require it and must not fake it. This wrap slice does not change that.
...
4. Key Vault (`terraform/s3rch`) — already the production path.
