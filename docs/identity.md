...
| `components/RoomsList.tsx` | Quiet Mine / Public / Network rooms list + New room (SIWE, Mine only). Network rooms = shared `s3rch/rooms`. Not a `/rooms` landing. Copy: live chat and presence are this pass; WebRTC attempted over STUN; meetings / streams still later |
