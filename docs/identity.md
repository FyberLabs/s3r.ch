...
- Meetings, live streams, hop UI, Elect / wills / Case C. Live chat and presence over Gun subscriptions ship; they are not a TURN/WebRTC mesh. `gun/lib/webrtc` + STUN ships; path A allocate hops when env is set. Durable graph stays requirements-only: [durable-graph-and-turn.md](durable-graph-and-turn.md).
