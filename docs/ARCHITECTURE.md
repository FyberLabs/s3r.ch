...
`FeedTab = "public" | "mine" | "network"` — Public, Mine, and Network render. Public keeps snapshot hydrate + shared. Network is the live mesh view (seed peer / WebRTC). Same FeedItem / Room shapes and tags-first ranker. No `/api/network`.
...
- Live mesh delivery of granted objects. Meetings / streams. Unshare / HAM-delete. Network tab **does** ship (live Gun subscriptions; not a finished P2P mesh claim).
