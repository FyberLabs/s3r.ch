...
| **Share-into-mesh** | User chooses to publish an **admitted** native post onto `gun.get('s3rch').get('items')`, an **admitted** room node onto `gun.get('s3rch').get('rooms')`, or an **admitted** user node / selected claims onto `gun.get('s3rch').get('users')` | Wired for own native posts, own rooms, and own user node / claims (confirm + put). Room share ≠ post share. User-node share ≠ dumping every held claim. **Network** reads live item/room puts (and the seed graph) via Gun subscriptions. Shared users are a quiet Discover provenance line, not a Popular list. Not unshare. Not OutboundAdapter. No `/api/users` | Live mesh **delivery** of granted objects. Unshare. Overlay ingest still stays mine unless a later share lands |
...
- Putting a wallet's public GI traces in the public cache ≠ publishing that person's private overlay. Explicit share-into-mesh still required. Unshare is later.
...
- Live mesh delivery of granted objects. Meetings / streams. Unshare / HAM-delete. Network tab **does** ship (live Gun subscriptions; Discover reads that corpus). Gun user node + explicit claim share **do** ship. Not a finished P2P mesh claim.
