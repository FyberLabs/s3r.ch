...
| ATProto / Bluesky | yes | yes | public seeder + signed-in browser pull via AppView; `/api/ingest` CORS proxy. Explicit SIWE outbound via PDS `createRecord` (server app password) |
...
| Farcaster | yes (Hub HTTP + GI) | yes (where APIs exist) | public seeder + signed-in browser pull via Hubble HTTP; `/api/ingest` CORS proxy. Explicit SIWE outbound via hub `submitMessage` (server signer) |
...
- ActivityPub / Nostr **outbound** posting (inbound pull is wired; posting is not). Farcaster / ATProto outbound **does** ship (explicit Mine control; not auto-bridge). A browser extension CORS bypass is a different PR. RSS3 GI still has no public DNS.
