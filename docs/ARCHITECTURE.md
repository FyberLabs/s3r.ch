...
5. **Needed services live in Panopticon.** Same pattern as Hypermesh on Panopticon: an open product network with public APIs so others can run or sell equivalents in a market. Do not grow s3r.ch Azure into that service. Do not invent a second control plane. Azure is not the Gun datastore. Hypermesh Phase 1 Stripe / Keycloak locks stay in hypermesh-docs; this file does not move them. The SociACL crate stays in [FyberLabs/SociACL](https://github.com/FyberLabs/SociACL). This repo consumes Panopticon TURN allocate (path A), oracles attest v0, and payments receipt/intent from the Next server. An optional **time-boxed** infra coturn (path B) is allowed only with the **same TURN/URI contract** as the Panopticon product and a **DNS/config cutover** — not a permanent second plane. See [durable-graph-and-turn.md](durable-graph-and-turn.md).

6. **Payments hop, no paywall UI.** Access to content / connections will be crypto. This slice hops SIWE-gated `POST /api/payments/receipt` (and optional `/intent`) to Panopticon `POST /api/v1/payments/v0/receipt` / `/intent`. Empty env is fail-soft. Do not invent a SociACL grant from a receipt. A thinner fiat micropayment rail (us or others) is optional later. No Stripe, no token launch, no live feed paywall UI.

## End-state vs this slice

| | Now (this PR) | Next / end-state |
| --- | --- | --- |
| Who pulls Farcaster, ATProto, RSS, ActivityPub, Nostr, and optional RSS3 | Lab seeder on the container **and** signed-in browsers through `/api/ingest` **or** an allowlisted extension / localhost relay (same documented sources). Admit `GunFeedNode` `v: 1` onto Mine. Empty/failed sources write nothing. Direct browser-to-source still fails CORS in a naked tab | Same sources. Do not invent a second proxy or datastore. Azure is not the product graph |
...
| Identity | SIWE cookie session binds a checksummed address (EOA or ERC-1271 smart account). After auth, mainnet ENS, Polygon Unstoppable, plus Farcaster / Lens / RSS3 are held claims on `/feed` when bidirectional public lookups match (not login). **This slice** also holds session-gated email / phone confirm and a fixture KYC attestation as private claim ids (`email:…` / `phone:…` / `kyc:<issuer>:…`) after SIWE — never login. Live email/SMS send is env-gated (`CONFIRM_SEND_URL`); unset is honest "not configured". A real KYC issuer is env-gated (`KYC_ISSUER_URL`); unset keeps the fixture stub. Assembles/updates the Mine overlay `GunUserNode` with those linked claim ids (local graph / dest ACL / IndexedDB proof — not session-display-only). Public `s3rch/users/<wallet>` still requires an explicit share of the node or of one claim. Owner can unshare the user node (tombstone) or one claim (republish without that indicator). Overlay can still pull `GET /decentralized/{account}`; items already carry `author` / `provenance` | Same user node, HAM-merged. Holding is not publishing. |
...
now:
  ...
  gun/lib/webrtc after gun/browser (STUN ICE; TURN iceServers when allocate succeeds) — fall open if ICE fails
  snapshot hydration if the /gun socket is down — fail open
  user overlay → POST /api/ingest (CORS proxy) or s3rch-pull extension/relay → Mine only (not public seed)
  signed-in allowed-source pull → POST /api/ingest { allowedSource } or s3rch-pull → admitFeedNode → Mine (v: 1)
  explicit share-into-mesh (pulled items) → admit again → gun.get('s3rch').get('items') put
  explicit outbound (own native post) → SIWE POST /api/outbound → Farcaster submitMessage / ATProto createRecord
  share-into-mesh does not call outbound; outbound does not write Gun
...
next:
  path A allocate hop is wired — [durable-graph-and-turn.md](durable-graph-and-turn.md)
...
| **Outbound** | Holder chooses **Post to Farcaster** / **Post to Bluesky** on an owned native post. SIWE `POST /api/outbound`. Server env only: Farcaster `FARCASTER_FID` + `FARCASTER_SIGNER_KEY` (+ optional `FARCASTER_HUB_AUTH`) → hub `POST /v1/submitMessage`; ATProto `ATPROTO_IDENTIFIER` + `ATPROTO_APP_PASSWORD` (+ `ATPROTO_PDS_BASE`, default `bsky.social`) → `createSession` + `createRecord`. Never `NEXT_PUBLIC_*`. Missing creds / hub / PDS fail closed with a short reason. Azure is not the hub, PDS, or social datastore | Wired. Own native only. Share / grant / delivery / inbound pull do not post out. ActivityPub / Nostr / RSS / RSS3 outbound stay unimplemented. No second IdP | Per-user signers later. Not this slice |
...
This is **not** AML or legal KYC, **not** PII collection, and **not** "verified human" theater. We do not claim sybil resistance or uniqueness proofs. We do not invent a token or a fake passport product. This slice ships SIWE (EOA + mainnet ERC-1271 / EIP-6492) + a cookie session on `/feed`, plus mainnet ENS / Unstoppable / Farcaster / Lens / RSS3 held claims after that session exists, plus session-gated email / phone confirm and a fixture KYC attestation as **private** held claims (prove to holder ≠ publish), plus light Check see-grants (not login). Coinbase Smart Wallet is an **onramp** (passkey smart account, then SIWE) — not a second IdP and not email login. WalletConnect is a wagmi connector **gated** on `NEXT_PUBLIC_WC_PROJECT_ID` (empty keeps injected + Smart Wallet, no WalletConnect; it is not a new IdP). It does not ship ENS/Unstoppable/fname/Lens/RSS3/email/phone/KYC as login, a KYC form, or a passport upload. s3r.ch does **not** use Panopticon Keycloak as primary login. Backup OAuth uses that realm’s existing brokers and a separate cookie; Gun / forum ownership stays the SIWE address. Login rules live in [identity.md](identity.md). Hypermesh portal stays Keycloak; this kit is s3r.ch login and later a Hypermesh wallet door.
...
| **Nostr** | NIP-01 kind 1 REQ on a documented relay + pubkeys | Public seed. Server-side WebSocket. Browser uses `/api/ingest` or the allowlisted extension/relay. Not outbound |
...
| **Email / phone / KYC attestations** | Session-gated confirm after SIWE. Fixture KYC issuer; `KYC_ISSUER_URL` / `CONFIRM_SEND_URL` when a real vendor exists | Private held claims (`email:…` / `phone:…` / `kyc:<issuer>:…`). Not login. Not a passport. Public Gun only after explicit share |
...
| Third-party digital KYC attestations | Fixture issuer (`kyc:fixture:held`) after SIWE. Swap `KYC_ISSUER_URL` when a real issuer exists. Not a s3r.ch passport product | **Private** (held claim). Public only after explicit share of that claim id |
| Old-school email / phone confirmation | Session-gated confirm after SIWE. Proves the claim **to the holder**. Lab fixture code `000000` when live send is unset | **Private**. Confirming an email does **not** publish it |

Live vendor OTP / a real KYC HTTP issuer stay env-gated and fail soft as "not configured" when unset. Do not invent a fake issuer. Mesh-wide Check treats `email:` / `phone:` / `kyc:` claim ids like other held claims. Hop UI does not ship.
...
3. **Crossing a URL is an edge handoff, not a grant.** `/api/ingest`, the allowlisted extension / localhost relay, and the lab seeder fetch are handoffs. Fetching does **not** mint `see`. A successful HTTP 200 from GI or a feed is not `CHECK(see, …)` and does not enroll an accessor.
...
1. **Browser CORS.** Farcaster Hubble, ATProto AppView, ActivityPub actors, RSS3 GI, and most RSS/Atom feeds will not load cross-origin from `s3r.ch`. Nostr relays are WebSocket (NIP-01) on the seeder / `/api/ingest` process — a naked `/feed` tab still does not talk to those hosts. This slice ships signed-in browser pull **through** `/api/ingest` (same documented seeder sources) **and** an allowlisted MV3 extension / `127.0.0.1` relay ([pull-relay.md](pull-relay.md)). Direct browser-to-source still fails in a normal page. Do not disable CORS on those hosts. The extension is not a general URL proxy. `/api/ingest` stays valid and does not write the public seed.
2. **App Service WebSockets + HTTP/2.** This slice wires the browser to same-origin `/gun`. Cloudflare already passes `/gun` 101 Switching Protocols and Gun DAM (orange-cloud and straight to App Service); the previous “CF/ARR drop WS” line is not the live diagnosis. Snapshot remains the fallback if the socket dies. No Cloudflare terraform in this repo. This PR enables `gun/lib/webrtc` with STUN ICE and a session-gated allocate hop. It does not claim browsers already mesh. If WebRTC is unavailable or ICE fails, the seed peer / snapshot path is unchanged.
3. **STUN is not TURN.** Google public `stun.l.google.com:19302` is **STUN**, not TURN. Do not document Google as a free TURN server. Do not stand up TURN on App Service. ICE stays STUN unless the Next hop returns allocate `iceServers`. Empty `PANOPTICON_TURN_*` / 401 / 503 / network fail keeps STUN + `/gun`. Requirements and Panopticon-vs-infra: [durable-graph-and-turn.md](durable-graph-and-turn.md). Product end-state stays Panopticon (lock 5). Optional time-boxed infra coturn only with the same URI contract and DNS/config graduation — not a second control plane.
...
Outbound (this slice): signed-in holder posts an owned native body via `POST /api/outbound` `{ network: "farcaster" }`. Server reads `FARCASTER_FID` + `FARCASTER_SIGNER_KEY` (ed25519 hex, never `NEXT_PUBLIC_*`) and `POST`s a signed CastAdd protobuf to `{FARCASTER_HUB_BASE}/v1/submitMessage`. Optional `FARCASTER_HUB_AUTH` is HTTP Basic (`user:pass`) when the hub set `--rpc-auth`. Pinata's public hub is often read-only — a write miss is an honest hub error, not a fake success. Empty env → `Farcaster hub is not configured.` Inbound pull is unchanged.
...
Outbound (this slice): signed-in holder posts an owned native body via `POST /api/outbound` `{ network: "atproto" }`. Server reads `ATPROTO_IDENTIFIER` + `ATPROTO_APP_PASSWORD` (never `NEXT_PUBLIC_*`) and calls the **PDS** (`ATPROTO_PDS_BASE`, default `https://bsky.social`) `com.atproto.server.createSession` then `com.atproto.repo.createRecord` (`app.bsky.feed.post`). AppView (`public.api.bsky.app`) is 405 on createRecord — do not write there. The access JWT is per-request and is never written to Gun. Empty env → `Bluesky app password is not configured.` Inbound AppView pull is unchanged.
...
Default relay: `wss://nos.lol` (override `NOSTR_RELAY_URL`). NIP-01 `REQ` with `{ kinds: [1], authors, limit: 20 }`. Server-side WebSocket on the seeder / `/api/ingest` process. The allowlisted extension / localhost relay may open that documented `wss` itself. A naked `/feed` tab still does not. `wss://relay.damus.io` timed out from this lab network on 2026-09-09 — a dead relay writes nothing.
...
3. `/feed` still paints from `GET /api/feed` if the `/gun` WebSocket is down. The client tries the seed peer and attempts `gun/lib/webrtc` (STUN, plus allocate TURN when the hop succeeds); snapshot if the socket is down or ICE fails.
...
| RSS / Atom | yes | yes (feed file / ping) | public seeder + URL ingest + signed-in documented-feed pull; `/api/ingest` or allowlisted extension/relay |
| ActivityPub | yes | yes | public seeder + signed-in browser pull via actor outbox first page; `/api/ingest` or allowlisted extension/relay. Outbound not wired |
| ATProto / Bluesky | yes | yes | public seeder + signed-in browser pull via AppView; `/api/ingest` or allowlisted extension/relay. Explicit SIWE outbound via PDS `createRecord` (server app password) |
| Nostr | yes | yes | public seeder + signed-in browser pull via NIP-01 kind 1 REQ; `/api/ingest` or allowlisted extension/relay. Outbound not wired |
| Farcaster | yes (Hub HTTP + GI) | yes (where APIs exist) | public seeder + signed-in browser pull via Hubble HTTP; `/api/ingest` or allowlisted extension/relay. Explicit SIWE outbound via hub `submitMessage` (server signer) |
...
Outbound: `OutboundAdapter` is wired for **Farcaster** (hub `submitMessage`) and **ATProto / Bluesky** (PDS `createRecord`). SIWE session required. Credentials are server env only. Native compose and share-into-mesh do **not** auto-post. ActivityPub / Nostr / RSS / RSS3 outbound stay `UnimplementedOutbound`. `/feed` buttons are short verbs. Fail closed when creds or hubs are missing.
...
- Deploying coturn, meetings, live streams, or hop UI in this slice. Rooms as Gun threads plus live chat and presence over Gun subscriptions are this slice. The Network tab reads that live graph; it is not a finished P2P mesh product. Presence is not a TURN/WebRTC mesh. `gun/lib/webrtc` + STUN ships; path A allocate hops when env is set.
- ENS / Unstoppable / fname / Lens / RSS3 / email / phone / KYC as login, Farcaster SIWF, KYC form, passport upload, email/SMS as IdP, or claims of legal KYC / sybil resistance / uniqueness. WalletConnect is gated on `NEXT_PUBLIC_WC_PROJECT_ID` (see [identity.md](identity.md)); do not invent a project id.
