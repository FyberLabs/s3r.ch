...
3. **Strong object / API versioning.** Hostable and downloadable copies have to interoperate as the product evolves. GunFeedNode and GunRoomNode carry a numeric `v`. This slice writes **`v: 1`**. Missing `v` on old seed rows still reads (treat as v1). Unknown future versions fail closed (`fromGunNode` / `fromGunRoomNode` return null) — drop that node, not the whole feed. Sharing between app copies is this versioned Gun shape, not an undocumented JSON dump. Do not invent a REST versioning matrix or a central API host.
...
| **Share-into-mesh** | User chooses to publish an **admitted** native post onto `gun.get('s3rch').get('items')`, or an **admitted** room node onto `gun.get('s3rch').get('rooms')` | Wired for own native posts and own rooms (confirm + put). Room share ≠ post share. **Network** reads those live puts (and the seed graph) via Gun subscriptions. Not unshare. Not OutboundAdapter | Live mesh **delivery** of granted objects. Overlay ingest still stays mine unless a later share lands |

`FeedTab = "public" | "mine" | "network"` — Public, Mine, and Network render. Public keeps snapshot hydrate + shared. Network is the live mesh view (seed peer / WebRTC). Discover browses tags already on Public and that live Network corpus. Same FeedItem / Room shapes and tags-first ranker. No `/api/network`.
...
A room is `{ id, title, owner, tags, ts, provenance, v }` with csv tags on the wire. A chat message is `{ id, room, author, body, ts, v }`. A presence heartbeat is `{ room, address, ts, v }` (no held claims, no signatures). This slice writes `v: 1`. Missing `v` reads as v1. Unknown future `v` fails closed (`fromGunNode` / `fromGunRoomNode` / `fromGunChatNode` / `fromGunPresenceNode` return null). Room object id for Check is the room id / `s3rch/rooms/<encodeKey(id)>`. Chat object id is `s3rch/rooms/<encodeKey(room)>/chat/<encodeKey(id)>`. Presence object id is `s3rch/rooms/<encodeKey(room)>/presence/<encodeKey(address)>`. Rooms, room posts, room chat, and room presence are **not** written into the public seed / `GET /api/feed` snapshot / lab seeder. Public rooms are client Gun `s3rch/rooms` after an explicit share. Chat and presence on a Mine-only room stay overlay until that room node is shared. Unsigned visitors can read public-room chat and presence when the room node is on the public graph; SIWE is required to send chat or announce presence. Do not put SIWE signatures or SEA `priv` / `epriv` on the chat or presence node.
...
- Live mesh delivery of granted objects. Meetings / streams. Unshare / HAM-delete. Network tab **does** ship (live Gun subscriptions; Discover reads that corpus). Not a finished P2P mesh claim.
- Browsers pull allowed sources (through proxy / relay / extension) and HAM-merge.
- Gun user node keyed by the SIWE address; browsers pull their own indicators as **held claims**. Wire the SEA pair (not `recall` to sessionStorage) and PRF wrap after SIWE is proven. Mesh-wide claim publish is later — Discover is not that.
