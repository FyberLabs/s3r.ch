...
Landing, `/feed` hero, metadata, and helper lines are customer-facing (what a visitor can do). Architecture locks stay in this file and README. Do not recopy Gun / WebRTC / STUN / TURN / grant / unshare essays onto public pages. Empty Network / Granted when the seed socket is down still mean the same thing in code; the UI says a live connection is needed, not the lock sentence.
