...
import { sessionGatedIntent } from "@/lib/payments-access";

export const dynamic = "force-dynamic";
export const runtime = "nodejs";

/**
 * Session-gated hop to Panopticon `POST /api/v1/payments/v0/intent`.
...
export async function POST(request: Request): Promise<Response> {
  ...
  const result = await sessionGatedIntent({ sessionAddress, body });
